...
class CARLA_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_lidars = []
        add_radars = []
        instanceidx = [str(id) for id in range(1, self.seq_len+1) ] # time instances by seq_len
        ## data augmentation
        ...
        if not self.test:
            data['beam'] = []
            data['beamidx'] = []
            beamstr = self.dataframe['unit1_beam'][index].split('_')
            beamidx_list = [int(idx)-1 for idx in beamstr]
            for beamidx in beamidx_list:
