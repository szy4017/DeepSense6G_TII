...
from config_seq_30to5 import GlobalConfig
from model2_seq_30to5 import TransFuser
from data2_seq_30to5 import CARLA_Data
...
class Engine(object):
	...
	def train(self):
		...
		for data in pbar:
			...
			gt_beamidx = torch.stack(data['beamidx'], dim=1).to(args.device, dtype=torch.long)
			gt_beams = torch.stack(data['beam'], dim=1).to(args.device, dtype=torch.float32)
...
if not args.Test:
	for keywords in ['scenario32_30to5','scenario33_30to5','scenario34_30to5']:
		# createDataset(trainval_root+train_root_csv, trainval_root+keywords,keywords) # already exist
		print(trainval_root+keywords)

# Data
if not args.train_adapt_together and not args.Test:
